# fcache2 IPv6 / Unified Extension Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add flow6 (IPv6) and flowu (Unified IPv4/IPv6) variants to fcache2, with tests, benchmarks, and documentation.
...
**Spec:** `docs/superpowers/specs/2026-03-19-fcache2-v6-unified-design.md`
...
**Files:**
- Create: `samples/fcache2/include/flow6_cache2.h`
- Modify: `samples/fcache2/include/flow_cache2.h`

- [ ] **Step 1: Create flow6_cache2.h**

Copy structure from `samples/fcache2/include/flow4_cache2.h`. Replace all `flow4` → `flow6`, `fc2_flow4` → `fc2_flow6`. **Important:** Remove the `FLOW_CACHE_LOOKUP_STEP_KEYS` / `FLOW_CACHE_LOOKUP_AHEAD_STEPS` / `FLOW_CACHE_LOOKUP_AHEAD_KEYS` macro definitions — these are already defined in `flow_cache2.h` (the umbrella) and `flow4_cache2.h`. Do not duplicate them. Change key struct:
...
struct fc2_flow6_entry {
    ...
    uint64_t               last_ts;      /*  8B */
    RIX_SLIST_ENTRY(struct fc2_flow6_entry) free_link; /* 4B */
    uint16_t               slot;         /*  2B */
    uint16_t               reserved1;    /*  2B */
} __attribute__((aligned(FC2_CACHE_LINE_SIZE)));
```

Static assert: `sizeof(struct fc2_flow6_entry) == FC2_CACHE_LINE_SIZE`.

All API declarations: rename `fc2_flow4_` → `fc2_flow6_` with same signatures.

- [ ] **Step 2: Update flow_cache2.h umbrella**

Add `#include "flow6_cache2.h"` after the existing `#include "flow4_cache2.h"` in `samples/fcache2/include/flow_cache2.h`.

- [ ] **Step 3: Compile header to check syntax**

Run: `cd samples/fcache2 && echo '#include "flow6_cache2.h"' | gcc -std=gnu11 -I../../include -Iinclude -mavx2 -msse4.2 -fsyntax-only -xc -`
...
```bash
git add samples/fcache2/include/flow6_cache2.h samples/fcache2/include/flow_cache2.h
git commit -m "fcache2: add flow6 header with 64B IPv6 entry"
```

### Task 2: flow6 implementation

**Files:**
- Create: `samples/fcache2/src/flow6.c`
- Modify: `samples/fcache2/Makefile`

- [ ] **Step 1: Create flow6.c**

Copy `samples/fcache2/src/flow4.c` to `flow6.c`. Replace all occurrences:
- `fc2_flow4` → `fc2_flow6`
- `flow4` → `flow6`
- `FC2_FLOW4` → `FC2_FLOW6`

Key change in hash function — key size is now `sizeof(struct fc2_flow6_key)` (44B):
...
The `#include` at the top changes to `#include "flow6_cache2.h"`. The `RIX_HASH_GENERATE_STATIC_SLOT_EX` invocation is renamed to use `fc2_flow6_ht`, `fc2_flow6_entry`, `fc2_flow6_cmp`, `fc2_flow6_hash_fn` — this wires the hash table code generation to the flow6-specific functions.

Everything else (relief, maintain, lookup_batch, fill_miss_batch) is a mechanical rename.

- [ ] **Step 2: Update Makefile**

In `samples/fcache2/Makefile`:
- Add `$(INCDIR)/flow6_cache2.h` to `PUB_HDRS`
...
Run: `cd samples/fcache2 && make clean && make static`

Expected: builds `lib/libfcache2.a` containing flow4.o and flow6.o.

- [ ] **Step 4: Commit**

```bash
git add samples/fcache2/src/flow6.c samples/fcache2/Makefile
git commit -m "fcache2: add flow6 implementation (copy from flow4, 44B key)"
```

### Task 3: flow6 functional tests

**Files:**
- Modify: `tests/fcache2/test_flow_cache2.c`

- [ ] **Step 1: Add flow6 include**

Add `#include "../../samples/fcache2/include/flow6_cache2.h"` at top.

- [ ] **Step 2: Add flow6 make_key helper**

```c
static struct fc2_flow6_key
make_key6(unsigned i)
{
    struct fc2_flow6_key k;
...
```c
static unsigned
count_hits6(const struct fc2_flow6_result *results, unsigned n)
{
    unsigned hits = 0u;
...
test_flow6_timeout_boundary(void)
{
    ...
    struct fc2_flow6_entry pool[MAX_ENTRIES];
    struct fc2_flow6_cache fc;
    struct fc2_flow6_config cfg = { .timeout_tsc = 100u, .pressure_empty_slots = 1u };
    struct fc2_flow6_key keys[2];
    struct fc2_flow6_result results[2];
    uint16_t miss_idx[2];

    printf("[T] fc2 flow6 timeout boundary\n");
    fc2_flow6_cache_init(&fc, buckets, NB_BK, pool, MAX_ENTRIES, &cfg);
    keys[0] = make_key6(0);
    keys[1] = make_key6(1);

    /* Insert at t=1000 */
    fc2_flow6_cache_lookup_batch(&fc, keys, 2u, 1000u, results, miss_idx);
    fc2_flow6_cache_fill_miss_batch(&fc, keys, miss_idx, 2u, 1000u, results);

    /* At t=1099 (just before timeout): maintain should NOT evict */
    if (fc2_flow6_cache_maintain(&fc, 0u, NB_BK, 1099u) != 0u)
        FAIL("should not evict before timeout boundary");
    if (fc2_flow6_cache_nb_entries(&fc) != 2u)
        FAIL("entries should remain at timeout boundary - 1");

    /* At t=1101 (just after timeout): maintain SHOULD evict */
    if (fc2_flow6_cache_maintain(&fc, 0u, NB_BK, 1101u) == 0u)
...
Update success message: `"ALL FCACHE2 TESTS PASSED (flow4 + flow6)\n"`

- [ ] **Step 7: Build and run tests**

Run:
```bash
cd tests/fcache2 && make clean && make fc2_test && ./fc2_test
```
...
```bash
git add tests/fcache2/test_flow_cache2.c
git commit -m "tests: add flow6 functional tests for fcache2"
```
...
**Files:**
- Create: `samples/fcache2/include/flowu_cache2.h`
- Modify: `samples/fcache2/include/flow_cache2.h`

- [ ] **Step 1: Create flowu_cache2.h**
...
All API declarations: `fc2_flowu_` prefix.

- [ ] **Step 2: Update flow_cache2.h umbrella**

Add `#include "flowu_cache2.h"` after flow6 include.

- [ ] **Step 3: Compile header to check syntax**

Run: `cd samples/fcache2 && echo '#include "flowu_cache2.h"' | gcc -std=gnu11 -I../../include -Iinclude -mavx2 -msse4.2 -fsyntax-only -xc -`

- [ ] **Step 4: Commit**

```bash
git add samples/fcache2/include/flowu_cache2.h samples/fcache2/include/flow_cache2.h
git commit -m "fcache2: add flowu header with unified IPv4/IPv6 key"
```

### Task 5: flowu implementation

**Files:**
- Create: `samples/fcache2/src/flowu.c`
- Modify: `samples/fcache2/Makefile`

- [ ] **Step 1: Create flowu.c**

Copy `samples/fcache2/src/flow6.c` (created in Task 2) to `flowu.c`. Replace:
- `fc2_flow6` → `fc2_flowu`
- `flow6` → `flowu`
- `FC2_FLOW6` → `FC2_FLOWU`
...
Add `$(INCDIR)/flowu_cache2.h` to `PUB_HDRS`, `$(LIBDIR)/flowu.o` to `OBJS`, and build rule:
...
```bash
git add samples/fcache2/src/flowu.c samples/fcache2/Makefile
git commit -m "fcache2: add flowu implementation (unified IPv4/IPv6)"
```
...
Include `flowu_cache2.h`. Add `make_keyu_v4()` and `make_keyu_v6()` helpers using `fc2_flowu_key_v4()` / `fc2_flowu_key_v6()`.

```c
static struct fc2_flowu_key
make_keyu_v4(unsigned i)
{
    return fc2_flowu_key_v4(
...
test_flowu_v4_v6_coexist(void)
{
    ...
    struct rix_hash_bucket_s buckets[NB_BK];
    struct fc2_flowu_entry pool[MAX_ENTRIES];
    struct fc2_flowu_cache fc;
    struct fc2_flowu_key keys[NB_V4 + NB_V6];
    struct fc2_flowu_result results[NB_V4 + NB_V6];
    uint16_t miss_idx[NB_V4 + NB_V6];
    unsigned miss_count;
    unsigned total = NB_V4 + NB_V6;

    printf("[T] fc2 flowu v4/v6 coexistence\n");
    fc2_flowu_cache_init(&fc, buckets, NB_BK, pool, MAX_ENTRIES, NULL);
    ...
    /* All miss initially */
    miss_count = fc2_flowu_cache_lookup_batch(&fc, keys, total, 1u,
                                               results, miss_idx);
    if (miss_count != total)
        FAILF("coexist initial miss_count=%u expected %u", miss_count, total);

    /* Fill all */
    if (fc2_flowu_cache_fill_miss_batch(&fc, keys, miss_idx, miss_count,
                                         10u, results) != total)
        FAIL("coexist fill failed");

    /* All hit */
    miss_count = fc2_flowu_cache_lookup_batch(&fc, keys, total, 20u,
                                               results, miss_idx);
    if (miss_count != 0u)
    ...
    if (!fc2_flowu_cache_remove_idx(&fc, results[0].entry_idx))
        FAIL("coexist remove v4 entry failed");
    miss_count = fc2_flowu_cache_lookup_batch(&fc, &keys[NB_V4], NB_V6, 30u,
...
Update success message: `"ALL FCACHE2 TESTS PASSED (flow4 + flow6 + flowu)\n"`
...
**Files:**
- Modify: `tests/fcache2/bench_flow_cache2.c`
- Modify: `samples/test/run_fc2_bench_matrix.sh`

Note: `tests/fcache2/Makefile` does not need modification — the bench binary links against `$(LIBFC2)` (the full `.a` archive), which already includes the new flow6.o and flowu.o from library Makefile changes in Tasks 2/5.
...
Add includes for `flow6_cache2.h` and `flowu_cache2.h` in `tests/fcache2/bench_flow_cache2.c`.

- [ ] **Step 2: Add flow6 key maker and cache context**

Add `bench_make_key6()` (IPv6 version) and `struct new6_cache_ctx` (with `fc2_flow6_cache`, `fc2_flow6_entry *pool`, `rix_hash_bucket_s *buckets`).
...
Each is a mechanical port: replace `fc2_flow4_` → `fc2_flow6_`, `flow4` → `flow6`, key maker → `bench_make_key6`.

- [ ] **Step 4: Add flowu key makers and cache context**

Add `bench_make_keyu_v4()`, `bench_make_keyu_v6()` using `fc2_flowu_key_v4()` / `fc2_flowu_key_v6()`.
...
In `samples/test/run_fc2_bench_matrix.sh`:
...
Run:
```bash
cd tests/fcache2 && make clean && make fc2_bench
./fc2_bench flow6 rate_fc2_only 65536 75 100 500000
./fc2_bench flowu rate_fc2_only 65536 75 100 500000
```
...
```bash
git add tests/fcache2/bench_flow_cache2.c samples/test/run_fc2_bench_matrix.sh
...
git commit -m "docs: update DESIGN for fcache2 flow6/flowu variants"
```
...
- v2 (fcache2): active development, 3 variants
...
git commit -m "docs: add samples README for fcache2"
```
