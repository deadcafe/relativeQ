...
fc2_flowu_hash_fn(const struct fc2_flowu_key *key, uint32_t mask)
{
    return rix_hash_hash_bytes_fast(key, sizeof(*key), mask);
}

int
fc2_flowu_cmp(const void *a, const void *b)
{
    return memcmp(a, b, sizeof(struct fc2_flowu_key));
}

#if defined(__AVX2__)
/*
 * fc2 currently targets an AVX2-focused flowu datapath.  Bind the staged
...
#endif
/*
 * Generate static cuckoo-hash helpers for flowu, including:
 *   fc2_flowu_ht_find / insert_hashed / remove / hptr
 */
RIX_HASH_GENERATE_STATIC_SLOT_EX(fc2_flowu_ht, fc2_flowu_entry,
                                 key, cur_hash, slot,
                                 fc2_flowu_cmp, fc2_flowu_hash_fn)
#if defined(__AVX2__)
#undef _RIX_HASH_FIND_U32X16_2
#undef _RIX_HASH_FIND_U32X16
#endif
...
static inline void
fc2_flowu_prefetch_insert_hash(const struct fc2_flowu_cache *fc,
...
static inline void
fc2_flowu_result_set_hit(struct fc2_flowu_result *result,
                         uint32_t entry_idx)
{
...
static inline void
fc2_flowu_result_set_miss(struct fc2_flowu_result *result)
{
    result->entry_idx = 0u;
}

static inline void
fc2_flowu_result_set_filled(struct fc2_flowu_result *result,
                            uint32_t entry_idx)
{
    result->entry_idx = entry_idx;
}
...
static inline void
fc2_flowu_update_eff_timeout(struct fc2_flowu_cache *fc)
{
...
fc2_flowu_threshold64(unsigned total_slots, unsigned parts64)
{
    return (unsigned)(((uint64_t)total_slots * parts64) >> 6);
}

static void
fc2_flowu_init_thresholds(struct fc2_flowu_cache *fc)
{
    fc->timeout_lo_entries = fc2_flowu_threshold64(fc->total_slots, 38u);
    fc->timeout_hi_entries = fc2_flowu_threshold64(fc->total_slots, 48u);
    fc->timeout_min_tsc = fc->timeout_tsc >> 3;
    if (fc->timeout_min_tsc == 0u)
        fc->timeout_min_tsc = 1u;
    fc->relief_mid_entries = fc2_flowu_threshold64(fc->total_slots, 45u);
    fc->relief_hi_entries = fc2_flowu_threshold64(fc->total_slots, 48u);
}

static inline unsigned
fc2_flowu_relief_empty_slots(const struct fc2_flowu_cache *fc)
{
    unsigned live = fc->ht_head.rhh_nb;
...
static inline struct fc2_flowu_entry *
fc2_flowu_alloc_entry(struct fc2_flowu_cache *fc)
{
    struct fc2_flowu_entry *entry = RIX_SLIST_FIRST(&fc->free_head, fc->pool);
...
static inline void
fc2_flowu_free_entry(struct fc2_flowu_cache *fc,
                     struct fc2_flowu_entry *entry)
{
    entry->last_ts = 0u;
...
static unsigned
fc2_flowu_scan_bucket_slots(struct fc2_flowu_cache *fc,
                            unsigned bk_idx,
                            ...
                            int *oldest_slot)
{
    ...
    struct fc2_flowu_entry *entries[RIX_HASH_BUCKET_ENTRY_SZ];
    ...
    for (unsigned slot = 0; slot < RIX_HASH_BUCKET_ENTRY_SZ; slot++) {
        ...
        if (idx == (unsigned)RIX_NIL)
            continue;
        entry = RIX_HASH_FUNC(fc2_flowu_ht, hptr)(fc->pool, idx);
        slots[used] = slot;
        entries[used] = entry;
        used++;
    }
    *oldest_slotp = -1;
    ...
    while (cur_count != 0u) {
        ...
        for (unsigned s = 0; s < cur_count; s++) {
            ...
            struct fc2_flowu_entry *entry = entries[idx];
...
static int
fc2_flowu_reclaim_bucket(struct fc2_flowu_cache *fc,
                         unsigned bk_idx,
                         uint64_t expire_before)
{
    struct fc2_flowu_entry *victim;
    unsigned removed_idx;
    unsigned expired_slots[RIX_HASH_BUCKET_ENTRY_SZ];
    unsigned expired_count;
    int victim_slot;

    expired_count = fc2_flowu_scan_bucket_slots(fc, bk_idx, expire_before,
                                                expired_slots, &victim_slot);
    if (expired_count == 0u)
        return 0;
    removed_idx = RIX_HASH_FUNC(fc2_flowu_ht, remove_at)(&fc->ht_head,
                                                         fc->buckets,
                                                         bk_idx,
                                                         (unsigned)victim_slot);
    RIX_ASSERT(removed_idx != (unsigned)RIX_NIL);
    victim = RIX_HASH_FUNC(fc2_flowu_ht, hptr)(fc->pool, removed_idx);
    RIX_ASSERT(victim != NULL);
    fc2_flowu_free_entry(fc, victim);
    return 1;
}

static unsigned
fc2_flowu_reclaim_bucket_all(struct fc2_flowu_cache *fc,
                             unsigned bk_idx,
                             uint64_t expire_before)
{
    unsigned expired_slots[RIX_HASH_BUCKET_ENTRY_SZ];
    ...
    evicted = fc2_flowu_scan_bucket_slots(fc, bk_idx, expire_before,
                                          expired_slots, NULL);
...
static unsigned
fc2_flowu_maintain_grouped_v2(struct fc2_flowu_cache *fc,
                              unsigned start_bk,
                              ...
                              uint64_t now_tsc)
{
    ...
    while (bucket_count-- != 0u) {
        ...
        reclaimed = fc2_flowu_reclaim_bucket_all(fc, cur_bk, expire_before);
...
static void
fc2_flowu_insert_relief_hashed(struct fc2_flowu_cache *fc,
                               union rix_hash_hash_u h,
                               uint64_t now_tsc)
{
    unsigned bk0, bk1;
    ...
    fc2_flowu_update_eff_timeout(fc);
    expire_before = (now_tsc > fc->eff_timeout_tsc) ?
        (now_tsc - fc->eff_timeout_tsc) : 0u;
    _rix_hash_buckets(h, fc->ht_head.rhh_mask, &bk0, &bk1, &fp);
    pressure_empty_slots = fc2_flowu_relief_empty_slots(fc);
    ...
    if ((unsigned)__builtin_popcount(hits_zero) <= pressure_empty_slots &&
        fc2_flowu_reclaim_bucket(fc, bk0, expire_before)) {
    ...
    if (bk1 != bk0) {
        ...
        rix_hash_arch->find_u32x16_2(fc->buckets[bk1].hash, fp, 0u,
                                     &hits_fp, &hits_zero);
        if ((unsigned)__builtin_popcount(hits_zero) <= pressure_empty_slots &&
            fc2_flowu_reclaim_bucket(fc, bk1, expire_before)) {
            fc->stats.relief_evictions++;
...
void
fc2_flowu_cache_init(struct fc2_flowu_cache *fc,
...
{
    struct fc2_flowu_config defcfg = {
        .timeout_tsc = UINT64_C(1000000),
        .pressure_empty_slots = FC2_FLOWU_DEFAULT_PRESSURE_EMPTY_SLOTS
    };
    ...
    fc->pressure_empty_slots = cfg->pressure_empty_slots ?
        cfg->pressure_empty_slots : FC2_FLOWU_DEFAULT_PRESSURE_EMPTY_SLOTS;
    fc2_flowu_init_thresholds(fc);

    RIX_SLIST_INIT(&fc->free_head);
    fc2_flowu_ht_init(&fc->ht_head, nb_bk);
    for (unsigned i = 0; i < max_entries; i++)
        RIX_SLIST_INSERT_HEAD(&fc->free_head, fc->pool, &fc->pool[i], free_link);
}

void
fc2_flowu_cache_flush(struct fc2_flowu_cache *fc)
{
    memset(fc->buckets, 0, (size_t)fc->nb_bk * sizeof(*fc->buckets));
    RIX_SLIST_INIT(&fc->free_head);
    fc2_flowu_ht_init(&fc->ht_head, fc->nb_bk);
...
unsigned
fc2_flowu_cache_nb_entries(const struct fc2_flowu_cache *fc)
{
    return fc->ht_head.rhh_nb;
}

unsigned
fc2_flowu_cache_lookup_batch(struct fc2_flowu_cache *fc,
    ...
    while (done < nb_keys) {
        for (unsigned lane = 0; lane < FC2_LOOKUP_CTX_PIPELINES; lane++) {
            ...
            case 0:
                ...
                RIX_HASH_FUNC(fc2_flowu_ht, hash_key)(&ctx[lane], &fc->ht_head,
                                                      fc->buckets,
                                                      &keys[key_idx[lane]]);
                stage[lane] = 1u;
                break;
            case 1:
                RIX_HASH_FUNC(fc2_flowu_ht, scan_bk)(&ctx[lane], &fc->ht_head,
                                                     fc->buckets);
                stage[lane] = 2u;
                break;
            case 2:
                RIX_HASH_FUNC(fc2_flowu_ht, prefetch_node)(&ctx[lane], fc->pool);
                stage[lane] = 3u;
                break;
            default: {
                unsigned idx = key_idx[lane];
                struct fc2_flowu_entry *entry =
                    RIX_HASH_FUNC(fc2_flowu_ht, cmp_key)(&ctx[lane], fc->pool);

                if (entry == NULL) {
                    fc2_flowu_result_set_miss(&results[idx]);
                    if (miss_idx != NULL)
                        miss_idx[miss_count] = (uint16_t)idx;
                    miss_count++;
                } else {
                    entry->last_ts = now;
                    fc2_flowu_result_set_hit(&results[idx],
    ...
    for (unsigned i = 0; i < total; i += step_keys) {
        if (i < nb_keys) {
            ...
            for (unsigned j = 0; j < n; j++) {
                RIX_HASH_FUNC(fc2_flowu_ht, hash_key)(&ctx[i + j], &fc->ht_head,
                                                      fc->buckets, &keys[i + j]);
            }
        }
        ...
        if (i >= ahead_keys && i - ahead_keys < nb_keys) {
            ...
            for (unsigned j = 0; j < n; j++)
                RIX_HASH_FUNC(fc2_flowu_ht, scan_bk)(&ctx[base + j],
                                                     &fc->ht_head,
                                                     fc->buckets);
        }
        ...
        if (i >= 2u * ahead_keys && i - 2u * ahead_keys < nb_keys) {
            ...
            for (unsigned j = 0; j < n; j++)
                RIX_HASH_FUNC(fc2_flowu_ht, prefetch_node)(&ctx[base + j],
                                                           fc->pool);
        }
        ...
        if (i >= 3u * ahead_keys && i - 3u * ahead_keys < nb_keys) {
            ...
            for (unsigned j = 0; j < n; j++) {
                unsigned idx = base + j;
                struct fc2_flowu_entry *entry = NULL;
#if FC2_LOOKUP_SPLIT_BK1
                ...
                while (hits0) {
                    ...
                    if (entry != NULL &&
                        fc2_flowu_cmp((const void *)&entry->key, ctx[idx].key) == 0) {
                        entry->last_ts = now;
                        fc2_flowu_result_set_hit(&results[idx],
                                                 RIX_IDX_FROM_PTR(fc->pool, entry));
                        hit_count++;
...
#else
                entry = RIX_HASH_FUNC(fc2_flowu_ht, cmp_key)(&ctx[idx], fc->pool);
...
unsigned
fc2_flowu_cache_fill_miss_batch(struct fc2_flowu_cache *fc,
                                const struct fc2_flowu_key *keys,
                                ...
                                struct fc2_flowu_result *results)
{
    ...
    for (unsigned base = 0; base < miss_count; base += FC2_FILL_PLAN_KEYS) {
        ...
        for (unsigned i = 0; i < n; i++) {
            ...
            hashes[i] = fc2_flowu_hash_fn(&keys[key_idx], fc->ht_head.rhh_mask);
            fc2_flowu_prefetch_insert_hash(fc, hashes[i]);
        }

        for (unsigned i = 0; i < n; i++) {
            unsigned key_idx = miss_idx[base + i];
            struct fc2_flowu_entry *entry;
            struct fc2_flowu_entry *ret;

            fc2_flowu_insert_relief_hashed(fc, hashes[i], now);
            entry = fc2_flowu_alloc_entry(fc);
            if (entry == NULL) {
                fc->stats.fill_full++;
                fc2_flowu_result_set_miss(&results[key_idx]);
                continue;
            }

            entry->key = keys[key_idx];
            entry->last_ts = now;
            ret = RIX_HASH_FUNC(fc2_flowu_ht, insert_hashed)(&fc->ht_head,
                                                              fc->buckets,
                                                              fc->pool,
                                                              entry,
                                                              hashes[i]);
            if (ret == NULL) {
                fc->stats.fills++;
                fc2_flowu_result_set_filled(&results[key_idx],
                                            RIX_IDX_FROM_PTR(fc->pool, entry));
                inserted++;
                continue;
            }
            fc2_flowu_free_entry(fc, entry);
            if (ret != entry) {
                ret->last_ts = now;
                fc2_flowu_result_set_filled(&results[key_idx],
...
unsigned
fc2_flowu_cache_maintain(struct fc2_flowu_cache *fc,
                         unsigned start_bk,
                         unsigned bucket_count,
                         uint64_t now)
{
    ...
    fc2_flowu_update_eff_timeout(fc);
    return fc2_flowu_maintain_grouped_v2(fc, start_bk, bucket_count, now);
}

int
fc2_flowu_cache_remove_idx(struct fc2_flowu_cache *fc, uint32_t entry_idx)
{
    struct fc2_flowu_entry *entry;
    ...
    if (entry == NULL)
        return 0;
    if (RIX_HASH_FUNC(fc2_flowu_ht, remove)(&fc->ht_head, fc->buckets,
                                            fc->pool, entry) == NULL)
        return 0;
    fc2_flowu_free_entry(fc, entry);
    return 1;
...
void
fc2_flowu_cache_stats(const struct fc2_flowu_cache *fc,
