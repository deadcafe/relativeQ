...
miss が多い batch では、`cache_insert_batch()` が hash を一度だけ
まとめて作り、その plan から candidate bucket を温めて、各 key を
再 hash せずに insert 段へ渡す。
...
| `src/body.h` | init、insert、insert_batch、lookup_batch、expire、stats、perf pkt-loop mix | `.c` ファイル |
...
`samples/test/fc_bench` で測定。entries=32768, nb_bk=4096, query=256。
各バリアント 50%（16384 エントリ）充填後に計測。
...
hit_lookup 37 cy（L1 ホット）は 3 GHz 換算で約 80 Mpps に相当。

### 15.2 メンテナンス：フルテーブルスイープ（fill=75%、全エントリ期限切れ）

| テーブルサイズ | flow4 cy/entry | flow6 cy/entry | flowu cy/entry |
|-------------|------:|------:|------:|
| 0.6 MB (1K bk) | 19 | 20 | 20 |
| 5.0 MB (8K bk) | 26 | 29 | 29 |
| 72 MB (64K bk) | 66 | 65 | 61 |

- **L2 内（≤5 MB）**: 16〜20 cy/entry、全バリアント同等。
  expire 時はキー比較不要（TSC チェックのみ）のため flow4 の
  インライン最適化は効かない。
- **DRAM（72 MB）**: 約 60〜66 cy/entry（約3倍の劣化）。
  ノード配列へのランダムアクセスが DRAM レイテンシに律速される。
  メモリ律速下でバリアント間の差は消失。

### 15.3 メンテナンス：分割スイープ（fill=75%、全エントリ期限切れ）
...
step サイズによる cy/entry の変動は ±15% 以内。カーソル管理のオーバーヘッドは
無視できるレベルであり、パケットバッチ間に少量ずつメンテナンスを挟んでも
効率が落ちないことを確認。

72 MB / step=64 の場合：約 47K cy/call ≈ 16 µs @3 GHz。パケットバッチ間に
十分収まる。
...
| 指標 | 値 | 備考 |
|------|-----|------|
| flow4 hit lookup | 37 cy | 3 GHz で約 80 Mpps |
| flow4 mixed 90/10 | 47 cy | 実運用ワークロード |
| flow4 miss fill | 110 cy | hash 計算 + insert 込み |
| メンテナンス (L2) | 16-20 cy/entry | バリアント非依存 |
| メンテナンス (DRAM) | 60-66 cy/entry | メモリレイテンシ律速 |
| 分割スイープオーバーヘッド | <15% | step サイズ非依存 |

- **flow4** はデータパス最速 — インライン CRC32C + XOR 比較によるディスパッチ排除が効果的
- **flow6/flowu** はデータパスで 26〜42% 劣るが、メンテナンス性能は同等
- **メンテナンスはエントリ数に線形スケール**。分割スイープはバッチ処理間に均等分散可能
...
- 公開 API は**サフィックスなし**の関数名
  （`fc_flow4_cache_lookup_batch` 等）。`fc_dispatch.c` の薄いラッパーが
  選択された ops テーブルに転送する。
- **ホットパス**（lookup, fill, maintain）は ops ポインタ経由でディスパッチ。
  **コールドパス**（init, flush, stats, remove, nb_entries）は `_gen` ビルドに
  直接転送。
...
付加される（例: `fc_flow4_cache_lookup_batch_avx2`）。マクロ未定義時は
元のサフィックスなし名が生成される。
...
SIMD find 操作（`_RIX_HASH_FIND_U32X16_2`）も同様に、`__AVX2__` 定義時は
`fc_cache_generate.h` がファイルスコープで AVX2 intrinsic に直接再バインドする。
...
```c
/* fc_ops.h の FC_OPS_DEFINE(flow4) が生成 */
struct fc_flow4_ops {
...
最適な ops テーブルを選択（AVX-512 > AVX2 > SSE > GEN）。
...
int main(void) {
    ...
    /* ... fc_flow4_cache_init(), fc_flow4_cache_lookup_batch() 等を使用 */
}
```
...
3. `_FC_OPS_SELECT_BODY` にプラットフォームの機能検出 API を使った
   判定ロジックを追加
4. `fc_ops.h` に `FC_OPS_DECLARE(flow4, _neon)` 等を追加
...
| ファイル | 役割 |
|----------|------|
| `fc_cache_generate.h` | `FC_CACHE_GENERATE` — 全関数をオプションサフィックス付きで生成; per-TU `rix_hash_arch` constructor 出力; `FC_OPS_TABLE` で ops テーブルインスタンス生成 |
| `fc_ops.h` | `FC_OPS_DEFINE` — ops 構造体定義; `FC_OPS_DECLARE` — extern 宣言; `FC_OPS_SELECT` — ランタイム CPU 検出; `FC_COLD_DECLARE` — コールドパス `_gen` extern; `fc_arch_init()` プロトタイプ |
| `fc_dispatch.c` | サフィックスなし公開 API ラッパー; `fc_arch_init()` 実装（`rix_hash_arch_init()` を吸収） |
| `flow4.c`, `flow6.c`, `flowu.c` | バリアント固有ハッシュ/比較 + `FC_CACHE_GENERATE` + `FC_OPS_TABLE` |
