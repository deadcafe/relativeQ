...
| **Bulk（ホットパス、パイプライン）** | |
| `fc_PREFIX_cache_find_bulk()` | パイプラインバッチ検索（ミス時挿入なし） |
| `fc_PREFIX_cache_findadd_bulk()` | パイプラインバッチ検索 + ミス時挿入 |
| `fc_PREFIX_cache_add_bulk()` | バッチ挿入（重複チェックなし） |
| `fc_PREFIX_cache_del_bulk()` | キー指定バッチ削除 |
| `fc_PREFIX_cache_del_idx_bulk()` | インデックス指定バッチ削除 |
| **Single-key（便利関数）** | |
| `fc_PREFIX_cache_find()` | 単一キー検索（find_bulk n=1 呼び出し） |
| `fc_PREFIX_cache_findadd()` | 単一キー検索+挿入（findadd_bulk n=1 呼び出し） |
| `fc_PREFIX_cache_add()` | 単一キー挿入（add_bulk n=1 呼び出し） |
| `fc_PREFIX_cache_del()` | 単一キー削除 |
| `fc_PREFIX_cache_del_idx()` | インデックス指定単一削除 |
| **Maintenance** | |
| `fc_PREFIX_cache_maintain()` | バケット範囲 reclaim |
| `fc_PREFIX_cache_maintain_step_ex()` | スキップ閾値付き部分スイープ |
| `fc_PREFIX_cache_maintain_step()` | 適応的シングルステップ maintenance |
| **Query（コールドパス）** | |
...
- `fcache` と同じ 4-stage batch lookup pipeline
  （hash_key_2bk → scan_bk_empties → prefetch_node → cmp_key_empties）
- staged fingerprint scan を AVX2 helper に直接バインド
- miss は cmp_key 段階で即座に inline insert（pipeline で計算済みの
  hash を再利用、bucket は L1 warm）。candidate bucket が高密度のときに
  local pressure relief を適用
