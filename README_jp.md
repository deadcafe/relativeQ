...
samples/
  ...
  fcache/          experimental action-cache 再設計 (flow4 のみ)
    include/
      flow_cache2.h          fcache 公開ヘッダの傘
...
`samples/fcache/` は、`fcache` の lookup pipeline を維持したまま、
...
- `fill_miss_batch()` は miss key を登録し、結果として得られた
  `entry_idx` を返す
- `fc_flow4_cache_maintain()` が、caller が指定した bucket 範囲だけを
...
- idle/background maintenance は grouped bucket walk を使い、
  ...
  を全件 `remove_at()` で回収する
- `fc_flow4_cache_stats()` で lookup/fill と local relief / maintenance の
  call/check/eviction を確認できる
- bucket / entry の prefetch は `rix_hash` の共通 helper に寄せてあり、
  lookup / insert / maintenance で同じ語彙で追える

`fcache` には built-in の hit-path governor も global expire walk も
...
現在の `fc` bench は、`tests/fcache/fc_bench` の引数付き mode を使うのが
前提です。
